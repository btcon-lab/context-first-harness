...
**선택 기능이지만 스킵은 조용히 넘기지 않는다.** 설치/연기/생략 중 하나로 결정하고 PROGRESS에 남긴다. 하네스 구성만 끝난 시점(구현 착수 전)은 대개 **연기** — 검사할 코드가 아직 없으므로, "구현 단계에서 제어 루프 설치"를 PROGRESS의 다음 할 일로 등재한다. 판단 기준표: `references/control-loop.md`의 "쓸지 말지 — 결정을 남긴다".
